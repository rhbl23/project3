...
res_random <- array(data=NA, dim=c(nnr, 105), dimnames=list(paste(1:nnr), c(
...
for (i in 1:length(nn)) {
  for (j in s:(s + replicate - 1)) {
    ...
    random_lasso <- RandomLasso(q1 = 50, q2 = 50, B = 'auto', 
                                random_state = 123, L = 30, threshold = 1/50)
    ...
    res_random[counter, 4:103] <- random_lasso$coef_[1:100]  # 1 intercept + 100 coefficients
    ...
    res_random[counter, 104] <- var(y_vector - y_pred)
    
    # Calculate and store Mean Squared Error (MSE)
    mse <- mean((y_vector - y_pred)^2)
    res_random[counter, 105] <- mse
