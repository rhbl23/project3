...
estimate_coef <- function(rl, bootstrap_number, q, method) {
  ...
  # Standardize X and y
  std <- standardization(X_bst, y_bst, family = rl$family)
  X_sc <- std$X_sc
  y_sc <- std$y_sc
  X_sd <- std$X_sd
  ...
  if (method == 'ElasticNet') {
    ...
    fit <- tryCatch({
      glmnet(X_sc, y_sc, family = rl$family, alpha = 0.5, weights = rl$sample_weight[bst_sample_idx],
    ...
    if (!is.null(fit)) {
      ...
      cv_fit <- tryCatch({
        cv.glmnet(X_sc, y_sc, family = rl$family, alpha = 0.5, weights = rl$sample_weight[bst_sample_idx],
                  standardize = FALSE, intercept = TRUE)
      }, error = function(e) { NULL })
    ...
    fit <- tryCatch({
      glmnet(X_sc, y_sc, family = rl$family, alpha = 1, penalty.factor = weights_adaptive, 
    ...
    if (!is.null(fit)) {
      # Select lambda that minimizes cross-validated error
      cv_fit <- tryCatch({
        cv.glmnet(X_sc, y_sc, family = rl$family, alpha = 1, penalty.factor = weights_adaptive, 
                  weights = rl$sample_weight[bst_sample_idx],
                  standardize = FALSE, intercept = FALSE)
      }, error = function(e) { NULL })
...
dim(X)
y <- factor(alon$y)
threshold <- 0.00001 # Threshold for selecting predictors

# Initialize the RandomLasso object with the threshold
random_lasso <- RandomLasso(q1 = 50, q2 = 50, B = 'auto', 
                            random_state = 123, L = 50, threshold = threshold, family="binomial")
start_time <- Sys.time()
# Fit the model
