...
estimate_coef_h <- function(hl, bootstrap_number, q, method) {
  ...
  bst_sample_idx <- sample(1:hl$n, size = hl$n, replace = FALSE)
  ...
  if (method == "ElasticNet") {
    fit <- cv.glmnet(X_sc, y_sc, family = hl$family, alpha = 0.5, weights = hl$sample_weight[bst_sample_idx], 
    ...
    fit <- cv.glmnet(X_sc, y_sc, family = hl$family, alpha = 1, penalty.factor = penalty_weights, 
